...
while [ "$rebuilding" ] ; do
    ...
    rebuilding=""
# Rebuild automation does not appear to quite work 
...
if fgrep -q '"state": "Success"' "$tmpfile" ; then
	...
	echo "NOTE: Build may take several minutes before new version is downloadable"
elif fgrep -q '"state": "Building"' "$tmpfile" ; then
    echo 'Rebuild on docker hub running'
    echo "NOTE: Build may take several minutes before new version is downloadable"
else
